...
type
  ...
  DrawRelays* = object
    ...
    imageSize*: proc (img: Image): tuple[w, h: int] {.nimcall.}
      ## Native pixel dimensions of a loaded image, or (0, 0) if invalid.
      ## Optional: drivers that predate it leave it nil (wrapper returns (0,0)).
...
proc imageSize*(img: Image): tuple[w, h: int] =
  if drawRelays.imageSize != nil: drawRelays.imageSize(img) else: (0, 0)
